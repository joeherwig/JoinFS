...
The P2P network protocol and the recording (`.jfs`) file format changed in 26.5.1.

- Every peer in a session, **and the hub / console you connect through**, must run 26.5.1 or newer. A hub still on an older build publishes corrupt positions to every client on it. 26.5.1 now *detects* a wrong-format position packet and discards it (keeping the last good position) instead of showing garbage, but the only real fix is to update every JoinFS - clients and hub - together.
- `.jfs` recordings saved by 26.5.1 or newer will **not** load in 26.5.0 or older. 26.5.1 still loads older recordings.
- A few `.jfs` files saved by the late-August 26.5.0 pre-release builds may already be misaligned; this release does not repair them.
- From 26.5.1 on, each position record carries a length prefix, so any future format addition will load (the unknown field is skipped) instead of throwing "Unable to read beyond the end of the stream".

## Bug Fixes

- **X-Plane: other aircraft are rendered again.** Since late August the X-Plane build listed remote traffic in the session window but never drew it - every position packet from the plugin failed with "Unable to read beyond the end of the stream". A ground-clearance field added to the shared position record for MSFS was gated on a version number, and the X-Plane plugin link's own protocol version happened to sit above that threshold, so JoinFS tried to read a field the native plugin never sends. The X-Plane path is now pinned to the exact byte layout the plugin speaks, and the position record was made self-describing (length-prefixed) so this class of mismatch can't recur. (Thanks @joeherwig for the contribution)
- **Console/hub: the WebSocket & webhook feeds are hardened.** A wrong-format or truncated position packet from a version-mismatched peer is now discarded (the aircraft keeps its last good position) instead of being decoded into garbage and re-broadcast to everyone. The WebSocket feed can no longer take the console process down: a serialization error is contained, a non-finite value serialises as `0` rather than throwing, a slow or half-open client is given a 5-second send deadline and then dropped instead of stalling delivery to every other client, and the change-tracking table no longer grows without bound. (Thanks @joeherwig for the contribution)
